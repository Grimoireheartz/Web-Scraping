    ...
    for (let x = 1; x < days; x++) {
        ...
        await con.connect(function (err) {
            ...
            while (countdata <= text_total) {
                countdata++;
                if (countdata <= 20) {
                    var sql = `INSERT INTO userlogin (text_cusSite, SerialMachine,OperatingTime,Utilization,City,Last_Update) VALUES ('${text_cusSite[countdata]}', '${SerialMachine[countdata]}','${OperatingTime[countdata]}','${Utilization[countdata]}','${City[countdata]}','${newdate.toLocaleString()}')`;
                }
                else if (countdata > 20) {
                    var sql = `INSERT INTO userlogin (text_cusSite, SerialMachine,OperatingTime,Utilization,City,Last_Update) VALUES ('${text_cusSite[countdata]}', '${SerialMachine[countdata]}','${OperatingTime[countdata]}','${Utilization[countdata]}','${City[countdata]}','${newdate.toLocaleString()}')`;
                }
