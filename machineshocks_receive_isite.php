...
for ($x = 0; $x < (count($SerialMachine_arr) - 1); $x++) {
    echo("success");
    $sql_insert_quotreq = "INSERT INTO bsc_isite_machineshocks (serialmachine,totalshocks,highshocks,mediumshocks,lowshocks,lockouts,optimeandhighshocks,site,city,machinefamily,model,insertdata) 
                            VALUES ('$SerialMachine_arr[$x]','$TotalShocks_arr[$x]','$HighShocks_arr[$x]','$MediumShocks_arr[$x]','$LowShocks_arr[$x]','$Lockouts_arr[$x]','$optimeandhighshocks_arr[$x]','$site_arr[$x]','$city_arr[$x]','$MachineFamily_arr[$x]','$Model_arr[$x]','$insertdata_arr[$x]')";
    $result = mysqli_query($link, $sql_insert_quotreq);
}
